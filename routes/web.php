...
Route::middleware([AuthChecker::class])->group(function () {
     ...
     $login_guard_path = Config::get('authenticator.login_guard_path');

    // Route for displaying the two-step verification page.
    Route::get($login_guard_path.'/verify-two-step', [AuthenticatorController::class, 'verify_two_step'])
    ...
    Route::post($login_guard_path.'/verify-two-step', [AuthenticatorController::class, 'verify_two_step_process']);
    
    // Route for displaying the QR code scanning page for two-step verification.
    Route::get($login_guard_path.'/scan-two-step', [AuthenticatorController::class, 'scan_two_step'])
         ->name('authenticator.scan'); // Name the route for easy reference.

    // Route for processing the QR code scan form submission.
    Route::post($login_guard_path.'/scan-two-step', [AuthenticatorController::class, 'scan_two_step_process']);
});
